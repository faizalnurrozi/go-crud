...
func (handler AdminHandler) Browse(ctx *fiber.Ctx) (err error) {
	// Get Query Param
	...
	// Database Processing
	uc := usecase.NewAdminUseCase(handler.UcContract)
...
func (handler AdminHandler) ReadByID(ctx *fiber.Ctx) (err error) {
	// Get Param
	ID := ctx.Params("id")

	// Database Processing
	uc := usecase.NewAdminUseCase(handler.UcContract)
	res, err := uc.ReadBy("a.id", ID, "=")
...
func (handler AdminHandler) Add(ctx *fiber.Ctx) (err error) {
	...
	input := new(request.AdminAddRequest)
...
func (handler AdminHandler) Edit(ctx *fiber.Ctx) (err error) {
	...
	// Parse & Checking input
	input := new(request.AdminEditRequest)
	if err := ctx.BodyParser(input); err != nil {
		return handler.SendResponse(ctx, ResponseWithOutMeta, nil, nil, err, http.StatusBadRequest)
	}
	if err := handler.Validate.Struct(input); err != nil {
...
func (handler AdminHandler) DeleteByID(ctx *fiber.Ctx) (err error) {
	// Get Param
	ID := ctx.Params("id")
